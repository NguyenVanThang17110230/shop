...
class Checkout extends Component {
    ...
    async saveOrder() {
        ...
        if (user.fullName === undefined || user.phone === undefined || stateAddress.province === undefined || stateAddress.district === undefined || stateAddress.ward === undefined || user.address === undefined) {
            toast.info("Hãy nhập đầy đủ thông tin trước khi thanh toán!")
        }
        else {
            await OrderService.createOrder(this.state.user).then(res => {
                if(res.status === 200){
                    toast.success("Đặt mua hàng thành công!")
                    this.firstSaveOrderDetail(res.data.createOrder.code)
        ...
        if (Cookies.get('expireAuth') === undefined) {
            ...
            let listener = window.addEventListener('message', (message) => {
                ...
                if (message.data.errorCode === 0) {
                    ...
                    toast.error("Bạn thanh toán chưa thành công vui lòng thanh toán lại")
                }
    ...
    showTotal = () =>{
        ...
        var x = 0;
        for(var i = 0; i < cart.length ; i++){
            const v = cart[i].quantity * cart[i].product.sellPrice
            x+=v;
        }
        return x;
    }
    render() {
        console.log("this123",this.props);
                                ...
                                <div className="container backgroundContainerCheckout fixed">
                                    ...
                                    <div className="card cardShadowTotal">
                                        <div className="card-body">
                                            ...
                                            <h3 className="textCenterTotal textPaddingTotal">{formatter.format(this.showTotal())}</h3>
                                        </div>
                                    </div>
